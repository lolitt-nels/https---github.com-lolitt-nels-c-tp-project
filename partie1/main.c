...
//realise par Bedrane Melissa et Abbas imene
...
liste Listepar(){
    ...
    do{L->data.taille = (rand()%300 + 100)*10; }while(L->data.taille == 0); //si taille=0 on cherche une nouv valeur
    L->data.etat= rand() %2 + 0;
    p = L;//p pointe sur la tete
    q = L;//q pointe sur la tete

    for(i=0;i<9;i++){ //boucle de creation des elements
             p=(node *)malloc(sizeof(node));
             p->data.adr= q->data.adr + q->data.taille;
              do{p->data.taille = ( rand()%200 )*10 ; }while(p->data.taille == 0);
             p->data.etat= rand() %2 + 0;;
 ...
 { x=defiler(F);
   ...
   if(pmin!= NULL){
         Q=(tab*)malloc(sizeof(tab));
                  ...
                  pmin->data.etat=2;   //mettre la partition a  occupe
                if ((pmin->data.taille)>x.taille ){
                        createreste( pmin , x );}
                  }
    ...
    while (i < *n) {
        ...
        while (p != NULL && trouve == 0) {
            if ((p->data.etat == 0) && ((p->data.taille) >= x.taille)) {
                ...
                *T = Q;
                trouve = 1;
                //2 veut dire occupe AVEC une partition
                p->data.etat = 2;
                if ((p->data.taille) > x.taille) {
                    createreste(p, x);
                }
 ...
 while( i< *n )
 {
   ...
   if(pmax!= NULL){
               ...
               Q=(tab*)malloc(sizeof(tab));
                   Q->id=x.id; Q->affect=pmax ;
                   Q->svt=*T; *T=Q;//affectation
                  pmax->data.etat=2;// devient occupe AVEC affectation
...
void supress(tab *e, liste L){
    ...
    while(q!= p){
       ...
       node* suiv=q->svt;
       if(q!=L){if(prec->data.etat==0){q->data.taille=q->data.taille + prec->data.taille; //prec->svt=q->svt; //free(q);
        }};
...
void decalage(liste L,tab * T){
    ...
    while (q != NULL){
        ...
        if(q->data.etat == 2){//partiton occupee trouvee
            ...
            if((p->data.etat==0)&&(p->data.taille)>=(q->data.taille)){
                ...
                p->data.etat = 2; //rendre la partiton dans laquelle on va decaler occupee
   ...
   file r;
    ...
    do{      //recherche du processus a suprimer
           ...
           //re-remplir la file
            ...
            printf("affichages apres supression : \n");
            ...
            affichageListepar(L);//affichage de la liste de partition
             ...
             getchar();
            dessineeer(L,m);
               printf("   affichage de la file apres supression \n ");
            Affichefile(&f);
            //rearanger les partiton et affichages des elements
            decalage(L, m);
            printf("  affichages apres avoir rearanger\n   voici la liste : \n");
            affichageListepar(L);
     ...
     switch (politique){
        ...
        while(count>0){ printf("\n iteration numero %d \n", count);
           fileP ri;
            ...
            printf("\n execution du processus en cours..\n");
            ...
            decalage(L, table_2);
            printf("affichage apres rearangement \n");
            affichageListepar(L);
             printf("Appuiez sur un bouton pour voir l'etat de la liste apres rearangement:\n");
             getchar();
            dessineeer(L,table_2);
